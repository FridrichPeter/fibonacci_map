...
The analysis uses local [Natural Earth 1:50m data](assets/GEOGRAPHY.md) (242 country/territory features and 1,251 selected populated places). It needs no API key and makes no external reverse-geocoding calls. It analyzes term points, not the lines connecting them. Country/coastline boundaries are approximate; a nearest mapped city is a reference location, not an exact address or a claim that the point lies in that city. Unmatched points display **Open water / unmapped land**.
